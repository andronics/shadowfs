...
from shadowfs.rules.engine import (
    Condition,
    Rule,
    RuleAction,
    RuleEngine,
...
from shadowfs.rules.patterns import PatternType
...
class TestRuleEngine:
    ...
    def test_should_show_first_match_wins(self):
        ...
        engine.add_rule(Rule(action=RuleAction.EXCLUDE, patterns=["test_*.py"], priority=10))

        # Lower priority rule includes all Python files
        engine.add_rule(Rule(action=RuleAction.INCLUDE, patterns=["*.py"], priority=1))
    ...
    def test_enable_disable_rule(self):
        ...
        engine = RuleEngine()
        rule = Rule(action=RuleAction.EXCLUDE, name="exclude_pyc", patterns=["*.pyc"])
        engine.add_rule(rule)
